...
export function EditSnowOrder({ order, claims, setActiveTab, setReload }: Props) { 
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const supplyItems: SupplyItem[] = order.snowfrostCategory?.snowFrostItems.map(item => ({
                    category: "SNOWFROST",               
    ...
    async function handleSubmit() {
        try {
            ...
            const data = await SupplyOrderService.updateSnowOrder(updatedOrder, order.snowfrostCategory!.snowFrostOrderId);
