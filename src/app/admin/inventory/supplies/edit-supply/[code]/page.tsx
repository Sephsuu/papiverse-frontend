...
export default function EditSupply() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const data = await SupplyService.getSupplyByCode(String(params.code));
                setSupply(data);
    ...
    async function handleSubmit() {
        try {
            ...
            if (data) toast.success(`${supply.name} updated successfully.`)
        } catch (error) { toast.error(`${error}`) }
        finally { 
            setProcess(false); 
            setOpen(!open);
            ...
            <div className="w-150 mx-auto shadow-lg mt-[-20px] p-8 bg-white rounded-md max-md:w-full max-md:bg-light max-md:shadow-none">
                <div className="flex items-center gap-2">
                    <Image
                        ...
                        height={40}
                    />
                    <div className="font-semibold text-2xl">Edit Supply: <span className="text-darkorange">{ supply.name }</span></div>
                    <Image
